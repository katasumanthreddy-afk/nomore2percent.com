...
export default function RequirementsMap({ requirements }: { requirements: Requirement[] }) {
    ...
    <APIProvider apiKey={apiKey}>
      <div className="h-[600px] rounded-xl overflow-hidden border border-stone-200 relative">
        <Map defaultCenter={center} defaultZoom={10} mapId={mapId} disableDefaultUI={false} gestureHandling="greedy" onIdle={(e) => setMapInstance(e.map)}>
          {requirements.map((r) => (
            <AdvancedMarker key={r.id} position={{ lat: r.lat, lng: r.lng }} onClick={() => setActiveId(r.id === activeId ? null : r.id)}>
          ...
          {mapInstance && requirements.map((r) => (
            <RadiusCircle key={`circle-${r.id}`} map={mapInstance} center={{ lat: r.lat, lng: r.lng }} radius={r.radius_max_m} color={STATUS_COLOR[r.status] || '#f97316'} />
          ))}
        </Map>
        {active && (
