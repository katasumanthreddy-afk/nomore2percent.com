...
export interface PropertyFormData {
  title: string; description: string; price: string; price_per_sqft: string; area: string;
...
export default function PropertyForm({
  ...
  const set = (k: keyof PropertyFormData, v: any) => setForm((prev) => ({ ...prev, [k]: v }));
  ...
  const generateDescription = async () => {
    ...
    try {
      const res = await fetch('/api/generate-description', {
        ...
        body: JSON.stringify({
          title: form.title, area: form.area, propertyType: form.property_type,
          listingType: form.listing_type, bedrooms: form.bedrooms, bathrooms: form.bathrooms,
          sqft: form.sqft, floor: form.floor, price: form.price,
          amenities: form.amenities.split(',').map((a) => a.trim()).filter(Boolean),
        }),
      });
      const data = await res.json();
      if (data.success) set('description', data.description);
      else setAiError(data.message || 'Failed to generate description.');
    } catch {
  ...
  // ─── EXISTING PHOTO HANDLING (edit mode only) ───
  const deleteExistingPhoto = async (imageId: number) => {
    if (!confirm('Remove this photo?')) return;
    await fetch(`/api/properties/images/${imageId}`, { method: 'DELETE' });
    setExistingPhotos((prev) => prev.filter((p) => p.id !== imageId));
  };

  const setExistingAsCover = async (imageId: number) => {
    ...
    setExistingPhotos((prev) => prev.map((p) => ({ ...p, is_primary: p.id === imageId })));
  };

  const uploadPhotosForProperty = async (id: number) => {
    const startIndex = existingPhotos.length; // new photos become cover only if no existing photos remain
    for (let i = 0; i < photos.length; i++) {
      ...
      await fetch('/api/properties/images', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ property_id: id, storage_path: urlData.publicUrl, is_primary: startIndex === 0 && i === 0 }),
      });
    }
  };
  ...
  const submit = async () => {
    ...
    const res = await fetch(mode === 'edit' ? `/api/properties/${propertyId}` : '/api/properties', {
    ...
    const id = mode === 'edit' ? propertyId! : data.property.id;
    if (photos.length > 0) await uploadPhotosForProperty(id);
  ...
  return (
    <div className="flex-1 bg-stone-50 text-stone-900 min-h-screen">
      <div className="bg-white border-b border-stone-200 h-14 px-7 flex items-center justify-between">
        <div className="font-bold">nomore<span className="text-orange-400">2%</span> <span className="text-xs text-stone-500 font-normal">/ {mode === 'edit' ? 'Edit Property' : 'Add Property'}</span></div>
        <Link href="/admin" className="text-sm text-stone-500 hover:text-orange-400">Back to Admin</Link>
      </div>

      <div className="max-w-2xl mx-auto px-6 py-8">
        <h1 className="font-serif text-xl font-bold mb-6">{mode === 'edit' ? 'Edit Property' : 'Add New Property'}</h1>
        ...
        <Section title="Description & Amenities">
          ...
          <Field label="Description" full>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              >
                {generatingDesc ? (<><span className="w-3 h-3 border-2 border-emerald-400 border-t-transparent rounded-full animate-spin" />Generating...</>) : 'AI Generate Description'}
              </button>
        ...
        <Section title="Property Photos">
          {mode === 'edit' && existingPhotos.length > 0 && (
            <Field label="Current Photos" full>
              <div className="grid grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                {existingPhotos.map((p) => (
                  <div key={p.id} className="relative aspect-[4/3] rounded-lg overflow-hidden bg-white border border-stone-200">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={p.storage_path} alt="" className="w-full h-full object-cover" />
                    {p.is_primary ? (
                      <span className="absolute top-1.5 left-1.5 bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded">COVER</span>
                    ) : (
                      <button type="button" onClick={() => setExistingAsCover(p.id)} className="absolute top-1.5 left-1.5 bg-black/70 text-white text-[10px] font-bold px-2 py-0.5 rounded opacity-0 hover:opacity-100 transition-opacity">
                        Set as Cover
                      </button>
                    )}
                    <button type="button" onClick={() => deleteExistingPhoto(p.id)} className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-black/70 text-white text-xs flex items-center justify-center hover:bg-red-600 transition-colors">✕</button>
                  </div>
                ))}
              </div>
            </Field>
          ...
          <Field label={mode === 'edit' ? 'Add More Photos' : ''} full>
            <div
            ...
            >
              ...
              <div className="text-xs text-stone-500 mt-1">JPG, PNG, WEBP. {existingPhotos.length === 0 && 'First photo becomes the cover image.'}</div>
            </div>
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-3 mt-4">
                {photos.map((p, i) => (
                  <div key={i} className="relative aspect-[4/3] rounded-lg overflow-hidden bg-white border border-stone-200">
                    ...
                    {existingPhotos.length === 0 && i === 0 && <span className="absolute top-1.5 left-1.5 bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded">COVER</span>}
        ...
        <div className="flex gap-3 mt-6 items-center">
          <button onClick={submit} disabled={saving} className="bg-orange-500 text-white rounded-lg px-6 py-2.5 text-sm font-bold hover:bg-orange-400 transition-colors disabled:opacity-50">
            {saving ? (uploadProgress || 'Saving...') : mode === 'edit' ? 'Save Changes' : 'Add Property'}
          </button>
