...
export default function PropertyDetailPage() {
  ...
  useEffect(() => {
    fetch('/api/properties')
      ...
      .then((data) => {
        if (data.success) {
          const found = data.properties.find((p: Property) => String(p.id) === String(params.id));
  ...
  if (loading) return <div className="flex-1 bg-stone-50"><Header /><div className="p-10 text-center text-stone-400">Loading...</div></div>;
  if (!property) return (
